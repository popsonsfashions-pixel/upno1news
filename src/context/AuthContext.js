...
export function AuthProvider({ children }) {
    ...
    const uploadProfilePicture = async (file) => {
        ...
        try {
            console.log('Starting profile picture upload to ImgBB...');

            // Get API Key from env or fallback
            const API_KEY = process.env.NEXT_PUBLIC_IMGBB_API_KEY || 'c6040a5b60210fb0bfa945798d1c5a20';

            // Convert to base64
            const base64 = await fileToBase64(file);
            const formData = new FormData();
            formData.append('image', base64.split(',')[1]);
            formData.append('name', `profile_${user.uid}`);
            ...
            const downloadURL = data.data.url; // Or data.data.display_url
            console.log('Image uploaded to ImgBB, URL:', downloadURL);
